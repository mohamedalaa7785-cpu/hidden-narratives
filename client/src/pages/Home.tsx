...
import { BookOpen, Compass, Mail, PlayCircle, ScrollText } from "lucide-react";
...
export default function Home() {
  ...
  useEffect(() => {
    document.title = "Hidden Narratives | Historical Analysis, Episodes, and Video Essays";
    const metaDescription = document.querySelector('meta[name="description"]');
    if (metaDescription) {
      metaDescription.setAttribute(
        "content",
        "Hidden Narratives publishes original historical analysis, long-form episodes, and video essays on power, institutions, and forgotten turning points.",
      );
    }
  }, []);
  ...
  const handleNewsletterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await newsletterMutation.mutateAsync({ email: newsletterEmail, language: "en" });
      setNewsletterEmail("");
      alert("Thanks for subscribing. Please check your inbox for confirmation.");
    } catch {
        ...
        <section className="px-4 py-14 md:py-20">
          <div className="mx-auto grid max-w-5xl gap-8 rounded-xl border border-amber-900/30 bg-slate-900/50 p-6 md:grid-cols-[1fr,auto] md:items-center md:p-10">
            ...
            <form onSubmit={handleNewsletterSubmit} className="flex w-full flex-col gap-3 md:w-[320px]">
              ...
              />
              <Button type="submit" className="bg-amber-600 font-bold text-black hover:bg-amber-700">
